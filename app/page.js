import Link from "next/link";
import { getProducts } from "@/lib/products";
import ProductCard from "@/components/ProductCard";
import { STORE } from "@/lib/store";
import { getLang } from "@/lib/lang";
import { getT, fmt } from "@/lib/i18n";

export const dynamic = "force-dynamic";

const FEATURE_ICONS = [
  "M12 3v6m0 0c-3.5 0-6 2.2-6 5.5S8.5 20 12 20s6-2.2 6-5.5S15.5 9 12 9zM9 6l3 3 3-3",
  "M12 3c3 4 6 7 6 11a6 6 0 11-12 0c0-4 3-7 6-11z",
  "M4 20c4-1 6-3 7-7 1 4 3 6 7 7M12 13V4m0 0L8 8m4-4l4 4",
  "M12 4v3m0 0l-6 2 2 6a4 4 0 004 0M12 7l6 2-2 6a4 4 0 01-4 0m-6 5h12",
  "M21 12a8 8 0 01-11.6 7.2L4 21l1.8-5.4A8 8 0 1121 12z",
  "M7 8h10M7 12h6m-8 8V6a2 2 0 012-2h10a2 2 0 012 2v14l-3-2-2 2-3-2-2 2-3-2-1 .7",
];

const SERVICES = [
  {
    key: "svc1",
    icon: "M3 21h18M5 21V8l7-5 7 5v13M9 21v-6h6v6M9 11h.01M15 11h.01",
  },
  {
    key: "svc2",
    icon: "M3 7h11v10H3zM14 10h4l3 3v4h-7zM7.5 20a1.5 1.5 0 100-3 1.5 1.5 0 000 3zm10 0a1.5 1.5 0 100-3 1.5 1.5 0 000 3z",
  },
  {
    key: "svc3",
    icon: "M12 3l7 4v5c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V7l7-4zM9 12l2 2 4-4",
  },
  {
    key: "svc4",
    icon: "M12 21a9 9 0 100-18 9 9 0 000 18zM3 12h18M12 3c2.5 2.5 3.8 5.6 3.8 9S14.5 18.5 12 21c-2.5-2.5-3.8-5.6-3.8-9S9.5 5.5 12 3z",
  },
];

export default async function HomePage() {
  const lang = await getLang();
  const t = getT(lang);
  const products = await getProducts();
  const highlights = [
    ...products.filter((p) => p.tag && p.stock > 0),
    ...products.filter((p) => !p.tag && p.stock > 0),
  ].slice(0, 4);
  const years = new Date().getFullYear() - Number(STORE.since);

  return (
    <>
      <div className="hero landing-hero">
        <div className="inner">
          <div className="eyebrow">{fmt(t.sinceLine, { since: STORE.since, place: STORE.place })}</div>
          <h1>{lang === "ta" ? STORE.tamilName : STORE.name}</h1>
          <div className="tamil-tagline">
            {lang === "ta" ? "Quality dals at direct mill prices" : STORE.tamilTagline}
          </div>
          <p>{fmt(t.heroPara, { since: STORE.since })}</p>
          <div className="cta-row">
            <Link href="/products" className="btn primary big">
              {t.ctaStock}
            </Link>
            <a
              className="btn big"
              href={`https://wa.me/${STORE.whatsapp}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              {t.ctaWhatsApp}
            </a>
          </div>
          <div className="hero-badges">
            {t.badges.map((s) => (
              <span className="hero-badge" key={s}>
                {s}
              </span>
            ))}
          </div>
        </div>
      </div>

      <div className="stats-strip">
        <div className="inner">
          <div className="stat-item">
            <div className="stat-num">{years}+</div>
            <div className="stat-label">{t.statYears}</div>
          </div>
          <div className="stat-item">
            <div className="stat-num">{products.length}+</div>
            <div className="stat-label">{t.statItems}</div>
          </div>
          <div className="stat-item">
            <div className="stat-num">7–9</div>
            <div className="stat-label">{t.statOpen}</div>
          </div>
          <div className="stat-item">
            <div className="stat-num">2 min</div>
            <div className="stat-label">{t.statBus}</div>
          </div>
        </div>
      </div>

      <section className="landing-section container">
        <h2 className="section-title">{t.servicesTitle}</h2>
        <div className="service-cards">
          {SERVICES.map((s) => (
            <div className="service-card" key={s.key}>
              <div className="service-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24">
                  <path d={s.icon} />
                </svg>
              </div>
              <h3>{t[`${s.key}Title`]}</h3>
              <p>{t[`${s.key}Text`]}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="landing-section container">
        <h2 className="section-title">{t.stepsTitle}</h2>
        <div className="steps-row">
          {[1, 2, 3].map((n) => (
            <div className="step-card" key={n}>
              <div className="step-num">{n}</div>
              <h3>{t[`step${n}Title`]}</h3>
              <p>{t[`step${n}Text`]}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="landing-section container">
        <h2 className="section-title">{t.whyTitle}</h2>
        <div className="features-grid">
          {t.features.map((f, i) => (
            <div className="feature-card" key={f.title}>
              <div className="feature-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24">
                  <path d={FEATURE_ICONS[i]} />
                </svg>
              </div>
              <h3>{f.title}</h3>
              <p>{f.text}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="landing-section container">
        <div className="section-head">
          <h2 className="section-title">{t.highlightsTitle}</h2>
          <Link href="/products" className="link-btn">
            {t.seeAll}
          </Link>
        </div>
        <div className="cards">
          {highlights.map((p) => (
            <ProductCard key={p.id} product={p} lang={lang} />
          ))}
        </div>
      </section>

      <section className="landing-section container">
        <div className="visit-card">
          <div>
            <h2 className="section-title">{t.visitTitle}</h2>
            <p className="visit-line">{t.address}</p>
            <p className="visit-line">{t.landmark}</p>
            <p className="visit-line">
              <strong>{t.hours}</strong>
            </p>
          </div>
          <div className="visit-actions">
            <a className="btn primary" href={`tel:${STORE.phone.replace(/\s/g, "")}`}>
              {fmt(t.callBtn, { phone: STORE.phone })}
            </a>
            <a
              className="btn"
              href={`https://wa.me/${STORE.whatsapp}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              {t.msgWhatsApp}
            </a>
          </div>
        </div>
      </section>
    </>
  );
}
